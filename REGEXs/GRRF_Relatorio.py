...
def regex_grrf_relatorio(contra_cheque, obj_response):
    if re.search(r'Demonstrativo do Trabalhador de Recolhimento FGTS Rescisório', contra_cheque) is not None:
        obj_response["Descricao"] = "GRRF_Relatorio"
        obj_response["Tipo"] = "99"
        obj_response["Cnpj"] = numberWithoutMask(
            findCnpj.search(contra_cheque).group())
        obj_response["CodigoReceita"] = findCodReceita.search(
            contra_cheque).group().split(" ")[2]
        ...
        obj_response["NisInteressado"] = findPisPasep.search(
            contra_cheque).group().split("\n")[2]
        return obj_response
    return None
