...
def regex_kit_admissao(pdfText, obj_response):
    if re.search(r'REGISTRO DE EMPREGADO', pdfText) is not None and re.search(r'Autenticar', pdfText) is not None:
        obj_response["Descricao"] = "Kit Admissao"
        obj_response["Tipo"] = "100"
        obj_response["NomeInteressado"] = findNome.search(
            pdfText).group().replace("Sr.(a) ", "")
        obj_response["Cnpj"] = numberWithoutMask(
            findCnpj.search(pdfText).group())
        obj_response["CpfCnpjInteressado"] = numberWithoutMask(
            findCpf.search(pdfText).group())
        dia, mes, ano = findDataAdmissao.search(
            pdfText).group().split("\n")[1].split("/")
