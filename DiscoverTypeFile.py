...
def find_type_file(pdf_data, arquivo, file_name):
    ...
    try:
        ...
        campos = ["Valor", "Juros", "Multa", "Total"]
        for campo in campos:
            val = obj_response.get(campo)
            if isinstance(val, str):
                try:
                    obj_response[campo] = int(val.replace(".", "").replace(",", ""))
                except:
                    obj_response[campo] = 0
            elif val is None:
                obj_response[campo] = 0
                
        if not obj_response.get("Vencimento") or obj_response["Vencimento"] == "":
            obj_response["Vencimento"] = None

    except Exception as e:
        print(f"Erro no saneamento de dados: {e}")
        pass
